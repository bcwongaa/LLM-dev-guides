...
Binding surface: [`RULES.md`](RULES.md). This file keeps the original ADR process prose.
...
- Day-to-day implementation choices (the domain guides cover those)
...
| Situation | Use |
|---|---|
| How to code / ship / secure | **domain guides** |
| “Why is this stack / split / ledger shape law?” | **decisions ADR** |
...
- greenfield **language / major framework / primary datastore** (after stack consult)
- **new deployable** or multi-repo split (architecture)
- **auth model** (session vs IdP, tenancy model) (security)
- **money / ledger** shape (append-only vs mutable balance, etc.) (data)
- public **API strategy** that clients will freeze on (contracts)
- adopting a **platform** as default (e.g. Datadog-everywhere, flag system) (observability/release)

**Do not** write an ADR for:

- routine feature work
- local refactors
- library picks that stack already treats as routine within the stack
- temporary flags or one-off experiments (unless they become permanent law)

When unsure: **ask** the author whether the choice deserves an ADR (protocol).
...
```text
guides/decisions/
  RULES.md                  ← binding process
  REFERENCE.md              ← this process prose
...
Greenfield stack still follows **stack + protocol consult**; the ADR **records** the accepted
...
- [ ] Does not restate whole the domain guides — points at guides where useful
