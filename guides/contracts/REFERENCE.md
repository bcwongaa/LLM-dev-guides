...
Binding surface: [`RULES.md`](RULES.md). This file keeps the original API and contract prose and examples.
...
|---|---|
| Handler/service code shape | [`../code-style/RULES.md`](../code-style/RULES.md) |
| Logical engines vs deployables | [`../architecture/RULES.md`](../architecture/RULES.md) |
| Framework choice (Nest, etc.) | [`../stack/RULES.md`](../stack/RULES.md) |
| Tables, migrations, DB invariants | [`../data/RULES.md`](../data/RULES.md) |
| Deep auth product rules, PII policy | [`../security/RULES.md`](../security/RULES.md) |
| Deploy flags / expand-contract of APIs in prod | [`../release/RULES.md`](../release/RULES.md) |
| How to test contracts | **testing** (`../testing/RULES.md`) |

In-process module calls (same deployable) are [`../architecture/RULES.md`](../architecture/RULES.md), not remote contracts. Do not invent
...
4. Breaking changes were avoided, versioned, or **approved** ([`../protocol/RULES.md`](../protocol/RULES.md) ask).
...
7. Tests cover the contract behavior you changed (`../testing/RULES.md`).
...
There are **many** valid options: in-process calls ([`../architecture/RULES.md`](../architecture/RULES.md)), message queues/buses, gRPC, and
...
Preference when a **remote** internal call is justified ([`../architecture/RULES.md`](../architecture/RULES.md) already approved a deployable
split):

```
prefer:  in-process public module API (same deployable)     → architecture
...
New internal transport or public protocol outside HTTP/WebSocket: **ask** ([`../protocol/RULES.md`](../protocol/RULES.md) / [`../stack/RULES.md`](../stack/RULES.md)).
...
(align [`../code-style/RULES.md`](../code-style/RULES.md) entity conversion and [`../data/RULES.md`](../data/RULES.md) persistent vs in-memory shape).
...
- Validate **before** domain logic relies on the value ([`../data/RULES.md`](../data/RULES.md) trust boundary).
...
breaking a published contract ([`../protocol/RULES.md`](../protocol/RULES.md)).
...
Align [`../data/RULES.md`](../data/RULES.md): do **not** expose raw internal DB keys as the public contract **by default**.
...
- **Null** (when allowed) → clear / set null per documented meaning ([`../code-style/RULES.md`](../code-style/RULES.md) / [`../data/RULES.md`](../data/RULES.md) null vs absent)
...
If there is no event bus yet, do not introduce one for style points ([`../architecture/RULES.md`](../architecture/RULES.md) / [`../stack/RULES.md`](../stack/RULES.md) ask).
...
4. Retain **raw payload** when needed for audit or reprocessing ([`../data/RULES.md`](../data/RULES.md) JSON-for-third-party).  
...
- **Authorization** is application logic (roles/permissions/ownership) — not “the client
  sent `role: admin`.”
- Do not put secrets in query strings or logs (**observability**, [`../security/RULES.md`](../security/RULES.md)).
- Deep product auth/PII rules: [`../security/RULES.md`](../security/RULES.md). This file only places the boundary.
...
- Prefer **in-process** module APIs inside one deployable ([`../architecture/RULES.md`](../architecture/RULES.md)) before any network.
...
- [ ] Public IDs thought through ([`../data/RULES.md`](../data/RULES.md))
...
- [ ] Tests cover contract behavior changed (`../testing/RULES.md`)
...
| Topic | Layer |
|---|---|
| Ask before breaking / greenfield | [`../protocol/RULES.md`](../protocol/RULES.md) |
| Handler structure, conversion | [`../code-style/RULES.md`](../code-style/RULES.md) |
| In-process vs remote | [`../architecture/RULES.md`](../architecture/RULES.md) |
| Framework | [`../stack/RULES.md`](../stack/RULES.md) |
| Storage shape, null, JSON retention | [`../data/RULES.md`](../data/RULES.md) |
| Testing contracts | **testing** (`../testing/RULES.md`) |
| Auth/PII product policy | [`../security/RULES.md`](../security/RULES.md) |
| Rollout of breaking APIs | [`../release/RULES.md`](../release/RULES.md) |
