...
Binding surface: [`RULES.md`](RULES.md). This file keeps the original language/framework/storage prose and examples.
...
(→ **architecture** / `../architecture/RULES.md`), not schema design (→ **data** / `../data/RULES.md`),
and not day-to-day code smell (→ **code-style** / `guides/code-style/`). Per-framework taste
...
- Do **not** rewrite existing code to a preferred stack “because the guide likes Nest /
  React / Postgres better.” Work with what is there (see code-style scope rules).
...
NO  Business logic in the database — no SQL functions / stored procedures for business
    ...
    code. (Also load-bearing for data.)
```
...
| **SQLite** | Embedded store for the small localhost-only tool shape (architecture §7) or single-process utilities. Not the default for multi-user production services — that is Postgres. |
...
### Access style (with code-style §19)
...
## 9. Errors: code-style and language convention

Code-style’s core merit stands: **unexpected failures should surface where they are detected**,
...
2. **Follow the language’s convention** rather than fighting it.
   ...
   - Rust: `Result` / `Option` at fallible boundaries; panic only for true programmer
     invariants.
3. Where code-style says “don’t return errors as values,” read that as **don’t return errors as
...
✗ Fight Go/Rust error idioms to satisfy a TS reading of code-style
...
- **Brownfield:** local codebase wins; no drive-by re-stack (code-style).
...
- [ ] **ADR drafted** per decisions (status: `proposed`) recording the accepted choice
...
- [ ] TS: npm + Node
- [ ] code-style post-code check still applies to the code written
...
| Topic | Layer |
|---|---|
| Code smell, decomposition, naming, get/ensureGet | **code-style** / `guides/code-style/` |
| Domain split, service boundaries, greenfield layout intent | **architecture** / `../architecture/RULES.md` |
| Language / framework / storage choice | **stack** / `../stack/RULES.md` (this file) |
| Tables, migrations, invariants; reinforce no domain-in-DB | **data** / `../data/RULES.md` |
| Wire contracts, HTTP/events | **contracts** / `../contracts/RULES.md` |
| Application and runtime data observability | **observability** / `../observability/RULES.md` |
| Testing | **testing** / `../testing/RULES.md` |
| Security / secrets | **security** / `../security/RULES.md` |
| Change and release | **release** / `../release/RULES.md` |
| Standing decisions (ADRs) | **decisions** / `../decisions/` |
| How agents open guides and ask vs decide | **protocol** / `../protocol/RULES.md` |
