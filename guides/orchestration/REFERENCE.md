...
Binding surface: [`RULES.md`](RULES.md). Authority, ask vs decide, and the done bar live in [`../protocol/RULES.md`](../protocol/RULES.md). This file keeps the original orchestration prose, examples, and brief templates.


Companion to `../protocol/REFERENCE.md`. Protocol is the **every-task core**; this file is the
...
A solo one-file bugfix does **not** need this file — protocol core covers it.

**Authority rules live in protocol** (ask vs decide, parent-as-authority, hard human items).
...
- **Small PR ≫ big PR** is the default. Prefer several focused PRs over one mega-diff.
  Split by concern when a change grows (code-style smallest change still applies inside each PR).
...
- Agent **opens / updates the PR**; **human merges** into the protected integration base
  (aligns release: human ships protected integration / prod paths). Do not force-push shared
...
Protocol states the rules; this section is the worked detail.
...
```
✓  Subagent: “Need stack for new worker — options A/B; recommend B per stack. Parent decide.”
...
Subagents inherit **the protocol** (scope, guides, git isolation). Prefer:
...
protocol to obey it — at minimum: path to **protocol** (or an embedded child brief that restates
...
with no protocol is a protocol failure by the parent.
...
- Goal:
- GUIDES_ROOT:
- Read first: protocol (+ orchestration if writing on a branch; code-style/testing/… as needed)
- Owned paths:
- Do not touch:
...
1. **Understand** user goal; open protocol + relevant domain guides.
...
5. **Brief** each write child with the template above (include protocol / GUIDES_ROOT, **base +
   merge target**).
...
| Write subagent without protocol / owned paths / merge target in brief | Protocol never enters context |
...
```
Adapter (tool) → protocol (every-task core) → this file (when branching / spawning) → the domain guides → code
```

Protocol owns ask-vs-decide and the done bar; this file owns the git/parallel mechanics around
them. **release** still owns expand/contract and prod deploy safety.
