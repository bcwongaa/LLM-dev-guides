# AGENTS.md — project map (Grok Build + shared)

**This file is a MAP.** Law lives in the LLM Dev Guides. Do not invent a parallel style or stack system.

Grok Build loads `AGENTS.md` (and also `CLAUDE.md` if present). Prefer **one** root `AGENTS.md` shared with Codex. If both `AGENTS.md` and `CLAUDE.md` exist, keep them equivalent maps — no drift.
...
## Grok Build–only notes

- Skills under `~/.grok/skills/` or project `.grok/skills/` are optional and task-scoped; they never override L\*.
- Project rules dirs (`.grok/rules/`, `.claude/rules/` when compat on) = project facts only, not a style fork.
- Prefer reading guide files from disk (`read_file`) rather than paraphrasing L1/L3 from memory.
