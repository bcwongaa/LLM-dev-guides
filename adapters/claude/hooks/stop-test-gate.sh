...
# converts protocol "tests not worse" from self-attestation into an enforced gate.
#
# Configure the command via GUIDES_TEST_CMD (e.g. in .claude/settings.json "env").
# No GUIDES_TEST_CMD → hook is a no-op (matches protocol "no test command exists" exception).
set -uo pipefail
...
if [ "$status" -ne 0 ]; then
  ...
  echo "Protocol definition of done: tests must not be worse than the recorded baseline." >&2
